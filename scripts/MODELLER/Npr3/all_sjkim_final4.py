...
#It begin4s from here
aln = alignment(env)
aln.append(file='all_align_begin4.ali')
aln.write(file='all_align_final4.ali', alignment_format='PIR')
aln.write(file='all_align_final4.pap', alignment_format='PAP')
aln.id_table(matrix_file='all_align_final4.mat')
aln.check()
...
#       Use a harmonic potential and X-Y distance group.
        rsr.add(forms.gaussian(group=physical.xy_distance,
                               feature=features.distance(at['CA:25'],
                                                         at['CA:30']),
                               mean=17.0, stdev=5.0))

a = MyModel(env,
            alnfile  = 'all_align_final4.ali',     # alignment filename
            #knowns=('3t97C', '3ghgA', '3ghgA', '3u0cA'),
            knowns=('4f54A'),               # codes of the templates
            sequence = 'Npr3',              # code of the target
...
for files in os.listdir('.'):
    if fnmatch.fnmatch(files, 'Npr3.B*.pdb'):
        print files
        mdl = model(env, file=files)
        mdl.rename_segments('A', 950)
        mdl.write(files)
...
#
#e = environ()
#a = alignment(e, file='all_align_1xks_final4.ali')
#
...
##pylab.legend(loc=0)
#pylab.savefig('all_dope_profile_final4_N133N.png', dpi=150)
